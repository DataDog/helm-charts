...
You can provide custom scripts that will be mounted in the runner container at `/home/scriptuser/`:

#### Inline Script Files

Add scripts directly in your `values.yaml`:
...
Reference existing ConfigMaps containing scripts:

```yaml
runner:
  scriptConfigMaps:
    - configMapName: "deployment-scripts"
      directoryName: "deploy" # Optional: mount in subdirectory
    - configMapName: "utility-scripts"
      directoryName: "" # Mount directly in /home/scriptuser/
```

Create the ConfigMap separately:

```bash
kubectl create configmap deployment-scripts \
  --from-file=deploy.sh=./scripts/deploy.sh \
  --from-file=rollback.sh=./scripts/rollback.sh
```
...
| runner.scriptConfigMaps                            | list   | `[]`                                                                                                                                                                                                                                                                                                                                                                                                                                            | References to kubernetes configmaps that contain scripts to be used by the Datadog Private Action Runner                                                                                                      |
