...
- Rename `kubernetesKubeServiceNewBehavior` to `kubernetesKubeServiceIgnoreReadiness`. _Note: This feature requires Cluster Agent `7.76.0` that is not released yet._
...
_**Warning:**_ From this version onwards, on GKE Autopilot, only one "datadog" Helm chart release is allowed by Kubernetes namespace due to the following new constraints:
