...
| datadog.instrumentationCrd.customWorkloadTargets | list | `[]` | Namespaced custom workload resources that DatadogInstrumentation can target. Each target and via entry requires apiVersion, kind, and the plural Kubernetes resource; via is optional. |
