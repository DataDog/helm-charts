...
| datadog.csi.enabled | bool | `false` | Enable datadog csi driver (Requires installation of Datadog CSI Driver https://github.com/DataDog/helm-charts/tree/main/charts/datadog-csi-driver) This feature is still in beta Requires version 7.67 or later of the cluster agent |
...
| otelAgentGateway.config | string | `nil` | OTel collector configuration |
| otelAgentGateway.configMap | object | `{"items":null,"key":"otel-config.yaml","name":null}` | Use an existing ConfigMap for DDOT Collector configuration |
| otelAgentGateway.configMap.items | string | `nil` | Items within the ConfigMap that contain DDOT Collector configuration |
| otelAgentGateway.configMap.key | string | `"otel-config.yaml"` | Key within the ConfigMap that contains the DDOT Collector configuration |
| otelAgentGateway.configMap.name | string | `nil` | Name of the existing ConfigMap that contains the DDOT Collector configuration |
...
| otelAgentGateway.containers.otelAgent.volumeMounts | list | `[]` | Specify additional volumes to mount in the otel-agent container |
