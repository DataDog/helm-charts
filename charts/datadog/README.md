...
| datadog.securityAgent.runtime.fimEnabled | string | `nil` | Enable Cloud Workload Security (CWS) File Integrity Monitoring Valid values are: - false: FIM is turned off. - null: FIM configuration is not set (by default, FIM is enabled by the agent if runtime is enabled). - true: FIM is turned on. |
