...
| registryMigrationMode | string | `"auto"` | Controls gradual migration of agent image pulls from third-party registries (GCR, ACR) to registry.datadoghq.com. When enabled, the operator injects DD_REGISTRY_OVERRIDE_* env vars so that agent images are pulled from registry.datadoghq.com for matching sites, unless the user has set a custom registry. This has no effect on sites not covered by the active overrides. More sites will be enabled by default in future helm-chart releases. "auto" (default): enable overrides for sites where migration is rolled out.   Currently enabled: ASIA region (ap1.datadoghq.com). "all": enable all per-site overrides (ASIA, DEFAULT, EU, AZURE). "" or unset: disable all overrides. |
