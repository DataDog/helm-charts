...
# Operator release - scoped to the Test_baseline_crd function (see
# test/datadog-operator/baseline_test.go) so a real Deployment template
...
update-test-baselines-operator-crd:
	helm dependency update ./charts/datadog-operator 2>/dev/null
	go test -C test ./datadog-operator -count=1 -run '^Test_baseline_crd$$' -args -updateBaselines=true
...
# datadogCSIDrivers is required even though these tests don't exercise CSI: the operator
# manager fails to start without the CRD present, and disabling the CSI controller instead
# (datadogCSIDriver.enabled=false) hits an RBAC self-escalation error in the DDA reconciler
# on operator <1.29. Revisit dropping this CRD once 1.29 fixes that RBAC gap.
...
setup-mapper-crds:
	...
	helm install datadog-crds ./charts/datadog-crds \
		...
		--set crds.datadogCSIDrivers=true \
		--wait --timeout 2m
