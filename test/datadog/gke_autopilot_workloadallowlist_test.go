...
func Test_autopilotWorkloadAllowlistConfigs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			assertions: verifyAutopilotWorkloadAllowlistConstraints,
		},
		{
...
func verifyAutopilotWorkloadAllowlistConstraints(t *testing.T, manifest string) {
	...
	for _, volume := range ds.Spec.Template.Spec.Volumes {
		// datadogrun emptyDir is not in the Datadog WorkloadAllowlist volume spec
		assert.NotEqual(t, "datadogrun", volume.Name,
			"volume datadogrun is not allowed by the Datadog WorkloadAllowlist; use pointerdir instead")
