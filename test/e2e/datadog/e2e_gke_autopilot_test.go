...
func TestGKEAutopilotSuite(t *testing.T) {
	...
	chartPath, err := filepath.Abs(filepath.Join("charts", "datadog"))
	if err != nil {
		t.Fatalf("Failed to get absolute path: %v", err)
	}
	t.Logf("Absolute chart path: %s", chartPath)

	e2e.Run(t, &gkeAutopilotSuite{}, e2e.WithProvisioner(gcpkubernetes.GKEProvisioner(gcpkubernetes.WithGKEOptions(gke.WithAutopilot()), gcpkubernetes.WithAgentOptions(kubernetesagentparams.WithGKEAutopilot(), kubernetesagentparams.WithHelmRepoURL(""), kubernetesagentparams.WithHelmChartPath(chartPath)), gcpkubernetes.WithExtraConfigParams(config))))
}
