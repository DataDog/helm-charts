...
func TestMergeMaps(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := yamlmapper.MergeMaps(map1Copy, map2Copy)
...
func TestCustomMapFuncs(t *testing.T) {
	...
	t.Run("customMapFuncs_dict", func(t *testing.T) {
		...
		for _, funcName := range expectedFuncs {
			t.Run(funcName+"_exists", func(t *testing.T) {
				_, exists := yamlmapper.CustomMapFuncs[funcName]
				assert.True(t, exists, "Custom map function %s should be registered", funcName)
			})
		}

		assert.Equal(t, len(expectedFuncs), len(yamlmapper.CustomMapFuncs), "Should have exactly %d custom map functions", len(expectedFuncs))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			customFunc, exists := yamlmapper.CustomMapFuncs[tt.funcName]
	...
	t.Run("non_existent_function", func(t *testing.T) {
		_, exists := yamlmapper.CustomMapFuncs["nonExistentFunc"]
...
func TestMakeTable(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := yamlmapper.MakeTable(tt.path, tt.val, mapNameCopy)
...
func TestMakeTableEdgeCases(t *testing.T) {
	t.Run("nil_value", func(t *testing.T) {
		mapName := map[string]interface{}{}
		result := yamlmapper.MakeTable("spec.global.site", nil, mapName)
	...
	t.Run("path_with_multiple_dots", func(t *testing.T) {
		mapName := map[string]interface{}{}
		result := yamlmapper.MakeTable("a.b.c.d.e.f", "deep_value", mapName)
	...
	t.Run("path_with_numeric_keys", func(t *testing.T) {
		mapName := map[string]interface{}{}
		result := yamlmapper.MakeTable("spec.containers.0.name", "agent", mapName)
